...
We now use our structure and evolution models to calculate the necessary conditions for gas giant formation by core accretion.   Adopting a median disk lifetime of 3 Myr, we require a core to accrete its atmosphere within the lifetime of the gas disk.  Our results for atmosphere growth times -- for a range of fixed core masses and disk conditions -- are presented in \S\ref{sec:tcross}.  Section \ref{sec:critcore} gives our results for critical core masses, the minimum masses which can accrete a massive atmosphere within the disk lifetime.

Our models will focus on giant planet formation between 5 and 100 AU.  The outer disk is of particular interest for direct imaging searches.  While the growth of atmospheres close to the star is also important, our cooling model is not appropriate for these hotter atmospheres.  Their thick radiative zones where dust sublimates would require a different treatment.atmos
...
\subsection{Crossover Time}
\label{sec:tcross}
The time to grow a massive atmosphere is well characterized by the crossover time, $t_{\rm co}$.  As in \S\ref{sec:coolingan}, the standard definition of $t_{\rm co}$ is the time for the atmosphere to grow equal to the core mass.   The precise atmospheric mass threshold used to define $t_{\rm co}$ is not very significant.  More important is that atmospheric growth starts to accelerate by the chosen mass, as was shown above.  As explained in \S\ref{sec:endoftime}, we thus can (and do) safely use a lower mass threshold for $t_{\rm co}$ if the cooling model breaks down somewhat before the crossover mass is reached.


%\subsubsection{Effects of Core Mass}
%\label{Mct}

\Fig{fig:tvsM} displays $t_{\rm{co}}$ for a range of core masses at $a=10$ AU in our fiducial disk. Atmospheres grow faster around more massive cores due to stronger gravity. Our analytical results in \Eq{eq:tcoolan} under-predict the steepness of the core mass dependence, which is somewhat expected due to the neglect of self-gravity in the analytic model.  For the parameters of \Fig{fig:tvsM}, the critical core mass that gives $t_{\rm co} = 3$ Myr is between $7$ and $8 \, M_\oplus$.  Before examining   $M_{\rm crit}$ in more detail, we consider how $t_{\rm co}$ depends on disk and atmosphere parameters.
...
%\vspace{-0.5in}
\caption{Time to grow an atmosphere of mass $M_{\rm{atm}}$ for cores with fixed masses between $5 M_{\oplus}$ and $14 M_{\oplus}$ (as labelled) at $a=10$ AU in our fiducial disk. The circles mark the crossover time where $M_{\rm{atm}} = M_{\rm c}$.   Larger core masses give faster atmospheric growth, both to a fixed atmosphere mass and to the crossover mass.}
%AY say this somewhere? (as in the bottom panel of \Fig{fig:Ltplot})
\label{fig:tvsM}
\end{figure}
