...
The effect of mean molecular weight is also shown in \Fig{fig:tvsMcomp}.  A lower $\mu$ gives longer growth times, as it results in a smaller atmosphere scale height and thus a weaker pressure gradient to oppose self-gravity, which accelerates growth.  For $\mu = 2.0$, which represents the idealized case of an H$_2$ atmosphere devoid of Helium, $t_{\rm run}$ increases by factors of $\sim 2 - 3$.  Thus fairly drastic changes in atmospheric composition are required for $\mu$ to significantly affect core accretion timescales.
...
%\vspace{-0.5in}
\caption{The critical core mass as a function of semi-major axis, for a disk lifetime of $3$ Myrs and two values of the mean molecular weight ($\mu = 2.35$ is standard for a Hydrogen-Helium Solar mixture). The decline in $M_{\rm c,crit}$ with distance is a robust result for standard disk models.  The analytic model, which neglects self-gravity, over-predicts the steepness of the decline.}
\label{fig:Mcvsa}
\end{figure}
