...
In section \S\ref{coolingan} we defined the crossover time $t_{\rm co}$ as the time elapsed for the atmosphere to reach the crossover mass at $M_{\rm atm}=M\co$. Here we explore the dependence of the crossover time on core, disk and gas parameters, and determine the critical core mass needed to initiate runaway gas accretion within the lifetime of the protoplanetary disk. In section \ref{Mct} we show how the crossover time varies with core mass for fixed disk parameters, demonstrating that higher mass cores achieve runaway accretion more quickly. We explore how the crossover time is affected by by the mean molecular weight of the nebular gas in section \ref{muopacity} and by the disk temperature and pressure in section \ref{TPeffects}. Finally, we determine the critical core mass in section \ref{critcore} and explore its dependence on the dust opacity.  
...
We generate atmosphere evolution profiles for cores of different masses under a fixed set of disk conditions. Figure \ref{fig:tvsM} displays $t_{\rm{co}}$ for $M_{\rm c}$ between 5 and 14 $M_{\oplus}$ at $a=10$ AU in our fiducial disk. The envelope grows faster if the core is more massive, as it can bind a gaseous atmosphere more quickly due to larger gravity. This is consistent with our analytic results in equation (\ref{eq:tcoolan}). As such, there is a minimum core mass required to form a planet within a fixed crossover time, e.g. the typical lifetime of the protoplanetary disk, which we defined as the critical core mass. We defer numerical estimates of the critical core mass to section \ref{critcore}.
...
\subsubsection{Role of Disk Temperature and Pressure}
\label{TPeffects}
...
The crossover time is shorter for lower disk temperatures and higher pressures. We discuss the two effects separately.

An increase in disk temperature results in a longer crossover time, as a higher temperature increases the dust opacity. As a result, photons diffuse through the radiative layer more slowly, producing a lower planetary luminosity. Our analytic model predicts $t_{\rm{co}} \propto T^{\beta+ 1/2}$ (see equation \ref{eq:tcoolan}), where $\beta$ is the constant in the opacity law (\ref{eq:opacitylaw}). For our choice of $\beta=2$, the crossover time is therefore strongly dependent on the opacity. Different opacity assumptions will result in a different scaling with temperature of $t_{\rm{co}}$.  We note that the power law dependence of the crossover time on disk temperature found in our analytic model breaks down in the numerical model for low $T\di$. Nevertheless, the trends predicted by the scaled analytic approximation in the left panel of Figure \ref{fig:TPeffects} are still informative. For the full numerical solution, a factor of $\sim 3$ reduction in the disk temperature results in an decrease of the growth time by a factor of $\sim 30$. The atmosphere evolution is thus highly dependent on the temperature in the protoplanetary disk.

From equations (\ref{eq:tcoolan}) and (\ref{eq:xi}), the crossover time is inversely proportional to disk pressure, and hence a higher disk pressure results in a shorter crossover time. Increasing the disk pressure by a factor of $\sim 100$ only reduces the crossover time by a factor of $\sim 2$.  The crossover time is therefore only weakly dependent on the nebular pressure.
...
We can now combine the results obtained in section \ref{Mct} to determine the minimum core mass to form a giant planet before the dissipation of the disk,. i.e. the critical core mass. Observations of protoplanetary disks show that they dissipate over a time scale of a few million years (e.g., \citealt{lagrange00}, \citealt{haisch01}, \citealt{goldreich04}). We adopt a disk life timescale of $\tau=3 $ Myrs and determine the critical core mass as a function of disk temperature, pressure and mean molecular weight of the nebular gas. 

\begin{figure}[h]
\centering
\includegraphics[width=0.5\textwidth]{../../figs/ModelAtmospheres/RadSelfGravPoly/PaperFigs/Mcrit_vs_a_3Myrs_new.pdf}
%\vspace{-0.5in}
\caption{The critical core mass for a disk lifetime $\tau \sim 3$ Myrs as a function of semi-major axis, for different mean molecular weight of the nebular gas. The scaled analytic results are plotted for comparison. The critical core mass is lower at larger stellocentric distances.}
\label{fig:Mcvsa}
\end{figure}

Figure \ref{fig:Mcvsa}  shows that the critical core mass decreases for larger separations, primarily because the disk temperature is lower in the outer disk. As shown in section \ref{Mct}, the atmosphere grows faster for lower temperatures, which offsets the need for a larger core mass. The critical core mass is larger for a lower mean molecular weight, consistent with the results derived in section \ref{Mct}.  Figure \ref{fig:Mcritopacity} shows the effect of opacity reduction on the critical core mass. For an opacity one order of magnitude lower than the standard ISM opacity, the critical core mass decreases by roughly a factor of two. 
