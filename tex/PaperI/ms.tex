...
\item Dust grains provide the opacity in the radiative zone. \textbf{For the cool temperatures in the outer disk, our assumption of an inner convective --- outer radiative structure self-consistently ensures that the radiative layer remains cool enough to avoid dust sublimation. \footnote{Other opacity choices are discussed in \S\ref{sec:op}}.} %which remains cool enough to avoid dust sublimation. \textbf{This follows self-consistently }
...
In the low mass regime, $M\pla < M_{\rm th}/\sqrt{3}$, the length scales order as $\RB< \RH<H\di$.  In this regime, many studies assume the atmosphere matches the disk conditions at $\RB$.  We use $\RH$ as the matching radius in all regimes.  
\textbf{We believe $\RH$ to be the best outer boundary choice, as it allows us to include the density perturbations outside the Bondi radius in the structure calculations. This is also consistent with the fact that, in hydrostatic models, the density at $\RB$ is higher than the background density (see R06). Nevertheless, since the density change is modest, these two boundary conditions give similar results.}
...
While $\RH$ is always the outer boundary in our structure calculations, we quote planet masses as the enclosed mass within the smaller of $\RB$ and $\RH$. \textbf{In contrast with the choice of outer boundary, which has physical consequences, we note that the choice of atmospheric mass is primarily a convention that does not affect atmospheric structure or evolution. Despite the possible confusion between two outer boundaries, we believe this to be the preferred location for several reasons.  Our choice of envelope mass is conservative, as it gives the smaller mass when $\RB < \RH$, and is consistent with previous works that use $\RB$ as the outer boundary (e.g., \citealt{ikoma00}, \citealt{pollack96}). The extra mass between $\RB$ and $\RH$, however large, is nebular gas weakly compressed at the disk midplane density, unlikely to remain bound to the planet if the disk were to dissipate quickly. }
...
In order to apply the two-layer cooling model analytically, we require expressions for the atmospheric structure.  Conditions at the RCB are crucial as they set the interior adiabat and the radiative losses from the interior. (Recall that luminosity generation in the radiative zone is neglected.)  \textbf{Rather than setting the outer boundary at $\RH$ as in the numerical model, we assume that our non-self-gravitating atmosphere matches onto the disk at a distance $r_{\rm fit}=n_{\rm fit} \RB$, where $n_{\rm fit}>1$ (see \App{iso} and R06). We note that the exact choice of $n_{\rm fit}$ is not important for the non-self-gravitating atmosphere structure, as it only has an order unity effect on the density at $\RB$. For $n_{\rm fit} \rightarrow \infty$, }we express the temperature and pressure of the RCB, at the radius $R\cb$, as 
...
The total energy is concentrated towards the core if $|e| \rho  r^3 \propto \rho r^2$ drops with increasing $r$.  This condition requires $\gamma < 3/2$, which our choice of $\gamma = 7/5$ satisfies, but $\gamma = 5/3$ (monatomic gas) would not. \textbf{While a realistic gas may become monatomic at the high temperatures near the core due to hydrogen dissociation, in this work we assume that the gas is diatomic throughout, with $\gamma=7/5$ and thus the bulk of its total energy concentrated near the core. The effect of a realistic EOS on the atmospheric energy distribution is discussed in Piso, Youdin, \& Murray-Clay (in prep, hereafter Paper II).}
...
To facilitate comparison with our numerical results, we rescale the analytic results as a rough correction for the neglect of self-gravity.  The goal of the rescaling is not to get a perfect agreement, but to aid in the interpretation of general trends.  To rescale, we simply assume that runaway occurs at a modified crossover mass, $M_{\rm atm} = f M\co$. \textbf{Here $f<1$, as growth is slower in our analytic model due to the neglect of self-gravity. For the purpose of this scaling, we are only interested in atmospheric growth up to the modified crossover mass defined above. We are thus not concerned about evolution to yet higher masses where the analytic approximation becomes inaccurate.} Mathematically, this choice of crossover mass amounts to replacing $\xi \rightarrow f\xi$ (cf. Equations \ref{eq:crit} and \ref{eq:Pcbc}).  We choose $f = 0.13$, which normalizes the analytic and numerical models at 10 AU, for standard parameters.  The revised scalings are
\begin{subeqnarray}
...
\textbf{The atmospheric structure is well approximated by our non-self-gravitating, analytic solutions.  Deep in convective zones, \Eq{eq:Tconvdeep} gives $T \propto r^{-1}$ and $P\propto r^{-1/\delad}$.  This behavior is seen in all solutions in \Fig{fig:profiles} for $r\ll R\cb$. Near the core, where $m(r) \approx M\co$, \Eq{eq:deep} gives an expression for the core temperature, $T\co=\frac{G M\co}{C_{\rm P} R\co}$, which holds regardless of the atmosphere mass, as shown in Figure \ref{fig:profiles}. As the RCB is approached, \Eq{eq:Tconvdeep} is no longer valid, particularly for large envelope masses. Instead, these high mass solutions show a slightly flatter profile in $T$ and also in $P \propto T^{1/\delad}$, as explained by \Eq{eq:TPsg}.}
...
\textbf{We now use our structure and evolution models to estimate the timescales and core masses necessary for giant planet formation, for our assumed disk model, nebular gas properties and dust grain opacity.}
Adopting a median disk lifetime of 3 Myr, we examine the conditions for a fixed mass core to undergo runaway gas accretion within the lifetime of the gas disk.  Our results for atmosphere growth times -- for a range of core masses and disk conditions -- are presented in \S\ref{sec:tcross}.  Section \ref{sec:critcore} gives our results for critical core masses, the minimum masses needed to accrete a massive atmosphere within the disk lifetime.
...
The effect of mean molecular weight is also shown in \Fig{fig:tvsMcomp}.  A lower $\mu$ gives longer growth times, because more cooling is required to compress the atmosphere.  This effect is explained by the increase in the atmospheric scale height $\propto 1/\mu$ while the Bondi radius decreases, $\RB \propto \mu$. \textbf{The larger atmospheric scale height due to a lower $\mu$ results in a larger pressure depth at the RCB, $P\cb$, and thus a lower $R\cb$, cf. \Eq{eq:PcbRcb}.} For $\mu = 2.0$, which represents the idealized case of an H$_2$ atmosphere completely devoid of Helium, $t_{\rm run}$ increases by factors of $\sim 2 - 3$.  Thus fairly drastic changes in atmospheric composition are required for $\mu$ to significantly affect core accretion timescales. \textbf{While we note that changes in the mean molecular weight of the gas also affect the adiabatic gradient $\delad$, we defer the discussion of the influence of $\delad$ on atmospheric evolution to Paper II.}
...
\textbf{A larger disk lifetime further reduces $M_{\rm crit}$. Recent studies have shown that gas disks may live up to $\sim$$10-12$ Myr \citep{bell13}. As $M_{\rm crit} \sim t\di^{-3/5}$ (cf. Equation \ref{eq:tcoolan}), a disk lifetime of 10 Myrs decreases the critical core mass by a factor of two.} 
...
\textbf{Our results are consistent with those of previous core accretion studies. The solution of \citet{stevenson82}, which considers fully radiative atmospheres with constant opacity, predicts that the critical core mass is lower for lower envelope opacity and higher mean molecular weight of the nebular gas. Additionally, \citet{stevenson82} finds that the critical core mass decreases weakly with increasing envelope radius (via a logarithmic factor), i.e. with increasing orbital distance. Both of the above qualitative predictions are reproduced by our result.}

\textbf{In the regime of no ongoing planetesimal accretion, \citet{ikoma00} find an approximate analytic expression for the envelope formation time (the equivalent of the runaway accretion time $t_{\rm run}$ in our models), }

\begin{equation}
\label{eq:tauenv}
\tau_{\rm env} \sim 3 \times 10^8 \Big(\frac{M\co}{M_{\oplus}}\Big)^{-2.5} \Big(\frac{\kappa}{1 \text{cm$^2$ s$^{-1}$}}\Big) \,\,\, \text{yr},
\end{equation}
\textbf{at $a=5.2$ AU, Jupiter's location. The result above reproduces the linear opacity dependence of the atmospheric evolutionary times in our model (see \S\ref{sec:timeev}).  Moreover, we can approximate $t_{\rm run}$ in our model at 5 AU as} 
\begin{equation}
\label{eq:trun5au}
t_{\rm run} \sim 5 \times 10^8 \Big(\frac{M\co}{M_{\oplus}}\Big)^{-2.4} \,\,\, \text{yr},
\end{equation} 
\textbf{for the opacity-law (\ref{eq:opacitylaw}). Our results are thus consistent with those of \citet{ikoma00} at 5 AU, both in terms of scaling and magnitude.}

\textbf{Time-dependent studies that incorporate planetesimal accretion generally find larger formation timescales and critical core masses than those of this work. \citet{pollack96} find an evolutionary time and crossover mass of $\sim$$7.5$ Myrs and $\sim$$16 M_{\oplus}$, respectively, for a MMSN disk model at 5.2 AU and interstellar grain opacity. These values, however, decrease to $\sim$$3$ Myrs and $\sim$$12 M_{\oplus}$ if planetesimal accretion is entirely shut off during the gas accretion phase. The core accretion model of \citet{HubBod05} for Jupiter's formation predicts an evolutionary time of 3.3 Myrs for a $10 M_{\oplus}$ core, for an interstellar opacity, which is also consistent with our result at 5 AU.}

%\textbf{Lastly, the decline in $M_{\rm crit}$ with semimajor axis is also found by R06 for planets forming at $\gtrsim 5$ AU. A quantitative comparison with our results requires detailed calculations of the }
...
On the other hand, non-negligible accretion of solids onto cores could continue for the lifetime of the gas disk. \textbf{For our fiducial disk model, the isolation mass, $M_{\rm iso} \propto a^{3/4}$, increases with orbital distance, and may thus become larger than $M_{\rm crit}$ at large separations. In this case, the ongoing accretion of solids increases the critical core mass by inhibiting atmospheric growth, and may prevent accumulation of a significant envelope before the disk dissipates. It follows that} relatively high mass cores would remain as ice giants, akin to Uranus and Neptune. \textbf{Non-traditional accretion models such as pebble accretion or scattering are thus needed in the outer disk in order to avoid slow growth to the isolation mass, which would stall atmospheric evolution.}  Ongoing direct imaging surveys and their successors \citep{hinz12, macintosh12} will help discriminate among the various planet formation pathways in the outskirts of protoplanetary disks.

\acknowledgements{We thank Ruth Murray-Clay for detailed feedback and skillful proof reading. We appreciate valuable comments from Eugene Chiang and Scott Kenyon.  ANY thanks Phil Armitage for stimulating conversations. 
