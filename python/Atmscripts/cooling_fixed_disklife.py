...
#Mct_a10_03_k10 = ph.t_vs_Mc_fixed_a(2. / 5, 0.3, a, returnt = 1, opacity = kdust10)
...
crit03_k10 = ph.Mc_vs_a_fixed_t(2. / 7, 0.3, 3 * 10**6, opacity = kdust10)

au00 = crit00[0]
au03 = crit03[0]
au03_k10 = crit03_k10[0]

Mcrit00 = crit00[1]
Mcrit03 = crit03[1]
Mcrit03_k10 = crit03_k10[1]



fig = plt.figure(figsize = (4.5, 4.5*2./3))
ax = fig.add_subplot(1,1,1)

savefig = 1

ax.semilogx(au03, Mcrit03, '-o', label = r'$\nabla_{\mathrm{ad}}=2/7$, $\mu=$' + str(mufn(Y03))[:4])
#ax.semilogx(au03, Mcrit03, '-o', label = r'$\kappa=\kappa_{\rm{dust}}$')
ax.semilogx(au00, Mcrit00, '-rs', label = r'$\nabla_{\mathrm{ad}}=2/7$, $\mu=$' + str(mufn(Y00))[:4])
#ax.semilogx(au03_k10, Mcrit03_k10, '--g^', label = r'$\kappa=\kappa_{\rm {dust}}/10$')
ax.set_xlabel('a [AU]')
ax.set_ylabel(r'$M_c [M_{\oplus}]$ ')
ax.set_xticks((1, 3, 5, 10, 20, 40, 60, 100))
ax.set_xticklabels(("1", "3", "5", "10", "20", "40", "60", "100"))
ax.set_xlim(0.8, 110)
#ax.set_ylim(3, 12)
...
if savefig == 1:
    plt.savefig(userpath + \
                '/figs/ModelAtmospheres/RadSelfGravPoly/PaperFigs/Mcrit_vs_a_3Myrs_new.pdf')
plt.show()
